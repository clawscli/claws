...
func TestRegistry_GetAliasesForService(t *testing.T) {
	...
	// Should include cfn only (cf is now an alias for cloudfront)
	if len(aliases) != 1 {
		t.Errorf("GetAliasesForService() returned %d aliases, want 1", len(aliases))
	}

	if aliases[0] != "cfn" {
		t.Errorf("GetAliasesForService() = %v, want [cfn]", aliases)
	}
}
