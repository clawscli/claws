...
- **Repo**: github.com/clawscli/claws (private, ready to go public)
- **Blockers**: None
- **Last Updated**: 2025-12-15
- **Code**: 550+ Go files, ~74,000 lines
- **Test Coverage**: internal/ ~60% avg, view/ 18.7% (TUI code)
- **Services**: 67 services, 151 resources
...
- **Cost Management**: risp, costexplorer, budgets
...
Options:
  ...
  --demo                 Demo mode (mask account IDs, hide tags)
  -l, --log-file <path>  Enable debug logging to file
  -h, --help             Show help message

Environment Variables:
  CLAWS_READ_ONLY=1|true   Enable read-only mode
  CLAWS_DEMO=1|true        Enable demo mode
```
...
- **Demo mode**: Added `--demo` flag and `CLAWS_DEMO` env var to mask account IDs and hide tags
- **Environment variables**: Added `CLAWS_READ_ONLY` and `CLAWS_DEMO` support
- **Bug fixes**: Fixed service browser not displaying after startup/region change
- **VPC→IGW navigation**: Fixed filter not working when navigating from VPC to Internet Gateways
- **SSM warning**: Disabled noisy SSM plugin warning at startup
- **risp service**: Added Reserved Instances (`:ri`) and Savings Plans (`:sp`)
- **ODCR**: Added ec2/capacity-reservations (`:odcr` alias)
